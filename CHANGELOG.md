...
- **`scan --classify --enrich=copilot|ollama|anthropic`: opt-in LLM suggestions for the LOW-confidence and STALE cases classification already finds (#55).** The deterministic pipeline is unchanged either way — `scaffold`'s name inference and `convert`'s mechanical rewrites run exactly as before, with zero LLM dependency, matching `AGENTS.md` §4 decision #9 ("No LLM dependency"). `--enrich` only ever runs when the flag is passed: omitting it never imports, constructs, or calls into the new `enricher` domain at all, which is what the opt-in gating test in `src/commands/__tests__/enrich.test.ts` pins directly rather than trusting by inspection. Three providers, one shared prompt: the GitHub Copilot CLI (`copilot`, if on `$PATH`, no API key), a local Ollama daemon (`ollama`, no API key), and the Anthropic API (`anthropic`, via `ANTHROPIC_API_KEY`). Every provider is fallible by design (`AGENTS.md` §4 decision #10) — a missing binary, an unreachable daemon, a missing key, or an uninstalled SDK is reported per-target as `{ ok: false, reason: "unavailable" | "error", detail }`, never a throw, so a run against a provider that isn't set up still prints the full deterministic report with "Enrichment unavailable for N entries" rather than crashing or hanging. `scan` stays read-only: a suggestion is surfaced as a new, additive `enrichment` field on the flagged declaration in the human table, `--report=json`, and `--report=md` — it is never written to a file. `@anthropic-ai/sdk` is a `devDependency`, imported lazily from inside the `anthropic` provider and listed in `build.config.ts`'s `externals`, so a consumer who never passes `--enrich=anthropic` neither downloads it nor ships it in the bundle (still under the 500 KB gzipped bound). The OpenAI half of the issue's "Anthropic / OpenAI" framing is deliberately out of scope here: the issue's own flag enum only names `anthropic`, and an equally-shaped OpenAI provider is recorded as a follow-up in `AGENTS.md` §11.

- **A prebuilt GitHub Action and Bitbucket Pipe, wrapping `check` and `scan --classify` so a consuming repo's CI doesn't have to hand-roll the `npx` invocation or its exit-code handling.** A composite action at the repo root (`action.yml`, `uses: miguelcolmenares/jsdoc-to-tsdoc@main`) covers the same surface `check` and `scan --fail-on-missing`/`--fail-on-stale` already ship: which command to run, `--cwd`, `--only`/`--exclude`, `--report`, and a Node version — and it propagates the CLI's own exit code (`0`/`1`/`2`/`3`) as the step's own success or failure rather than swallowing it. **The Bitbucket Pipe (`pipe/`) ships as source only** — a Dockerfile, `pipe.yml`, and `pipe.sh` reading the same flags from Bitbucket's standard pipe-variable env vars, verified by building the image and smoke-running it against this repo's own `src/` in CI — but it is **not yet published** to the Bitbucket Pipe marketplace or any Docker registry, since that needs the maintainer's own Docker Hub and Atlassian Marketplace accounts. `pipe/README.md` carries the exact publish checklist. Both wrappers are exercised in CI against this repo's own source: the action via a local `uses: ./` step, the pipe via `docker build` + a smoke run — the closest either gets to a real integration test without the accounts a live publish would need.
- **`merge-driver` — a git merge driver that auto-resolves the one line a long-lived escalation branch's rebase ever conflicts on.** `escalate` documents `tsdoc-require-2/require`'s `"warn"` → `"error"` flip as "the only line that ever conflicts on a long-lived migration branch" — two branches (or a branch and a rebased `main`) each escalate the rule independently, and a plain rebase stops for a human to resolve a conflict whose answer is always "whichever side says `error`". `resolveSeverityConflict` (new, in `escalator/`) is the pure, testable core: given the three versions git's merge-driver protocol provides, it resolves when the *only* difference is that one severity value — preferring `error`, since escalation only ever moves one direction — and refuses (returns `null`) the moment anything else on the file differs, including an `off` on either side. A thin `merge-driver` subcommand wraps it in git's file-based merge-driver protocol (`%O %A %B`); on refusal it hands the same three files to `git merge-file`, so the result is the ordinary conflict markers a rebase with no driver configured would have produced, never a silent guess. Wiring it in is a one-time local step documented in the README — a `.gitattributes` line can be committed, but the `git config merge.<name>.driver` registration cannot, since git deliberately does not let a committed file alone make a clone run an arbitrary command during merge. `init` does not wire this up automatically for the same reason.
- **`scaffold --members` documents an interface's (or type-literal alias's) individual members, not just its header.** Previously `scaffold` wrote one stub for `export interface HeroSectionProps` — "Hero section props." — no matter how many properties it declared; a wide, undocumented interface still needed every member filled in by hand. This was noted as deliberately out of scope in a `declaration-shape.ts` comment (`describeStatement`'s interface branch never split docs onto members), because member enumeration and per-declaration insertion were, at the time, two different code paths with nothing forcing them to agree on what counts as an addressable member. What made it safe to close now: `member-targets.ts` (built for relocating an existing `@property` description onto a member) and `insertion-location.ts` (built for placing a stub above a declaration) turned out to already answer both halves of the problem generically enough to reuse one level down — a new `member-declarations.ts` combines them into `MemberDeclaration`, and a new `member-stub-builder.ts` reuses `stub-builder.ts`'s own rendering and line-placement rather than re-deriving either. A member whose declared type is callable (a method signature, or a property typed as a function) gets `@param`/`@returns` tags the same way a function declaration would; a plain data property gets a noun-phrase summary — inferred purely from the member's name and declared type, never a guess at what it means. A member that already has its own doc comment is left alone, independent of whether the interface's own header is documented — the two are separate gaps. Applies equally to interfaces and type-literal aliases, since the scanner already treats the two identically for member purposes. **Off by default**: a per-member stub can multiply a run's output (one comment per property instead of one per declaration), the same reasoning that keeps `convert --promote-line-comments` opt-in. Closes #62.
- **`check` resolves `tsdoc.json` per file now, so a monorepo can give each package its own custom tags and scope instead of sharing one project-root file.** `createTsdocValidator(cwd)` always loaded exactly one config and reused it for the whole run — the assumption `init` and `check` both made and #57 named directly. Each file now walks up from its own directory toward the project root looking for the nearest `tsdoc.json`, the same model ESLint's flat config and `tsconfig.json` resolution both use, and falls back to the root's config when nothing nearer exists — which is exactly what a single-root project (no nested `tsdoc.json`) has always done, so that case is unchanged byte-for-byte: same validator behavior, same exit codes, same messages. The new `createTsdocValidatorResolver` caches the walk and the built validator per directory, so a package with hundreds of files under one config pays the filesystem and parse cost once, not per file. A broken config — root or nested — still aborts the whole run with exit `2` before anything is reported, on the reasoning that a config that failed to load makes every file under it as untrustworthy as the single-root case already treated a broken root config; the message now names which `tsdoc.json` failed when it isn't the root's. `--report=json` gains `tsdocConfigs`, the sorted list of every config actually applied, alongside the existing single-path `tsdocConfig`. **`init`, `convert`, `scaffold` and `escalate` are unchanged and still assume one project-wide config** — generating/patching a `tsdoc.json` and an ESLint flat config per workspace is a separately scoped problem ("which flat config applies to which workspace" has no equivalent nearest-ancestor answer) and stays tracked as follow-up work in `AGENTS.md` §11.
...
- **`main` now requires one approving review before anything merges**, closing the gap where any push with green checks could land without a second set of eyes — including an agent's own `gh pr merge`. Dependabot's minor/patch auto-merge (`auto-merge-dependabot.yml`) is the one exception, and stays working: a new step in that workflow approves the PR itself, using a personal access token (`secrets.AUTO_APPROVE_TOKEN`) rather than the default `GITHUB_TOKEN`, because GitHub blocks a workflow's own token from approving a pull request in the same repository. Repository admins can still merge without a review via an explicit override (`gh pr merge --admin`); the change is that the *default* path no longer can.
