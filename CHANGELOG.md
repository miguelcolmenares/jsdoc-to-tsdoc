...
- **The README claimed "there is no existing tool" without naming the two that come closest.** It now does: [`@enact/jsdoc-to-ts`](https://github.com/enactjs/jsdoc-to-ts), which is actively maintained and solves a different problem (generating `.d.ts` from a JavaScript codebase), and [`wvbe/experimental-jsdoc-to-tsdoc-tool`](https://github.com/wvbe/experimental-jsdoc-to-tsdoc-tool), a 2022 Deno prototype of the same migration that re-serializes comments from an AST and drops the tags it does not implement. The claim survives — neither automates the workflow end to end — but it is stated against the prior art instead of past it, and the second one is the clearest available argument for why this tool edits comments in place. Reading them turned up two real bugs, filed as #84 and #85.
