...
> **Status: Alpha (v0.1.0, in development).** The `scan` and `convert` commands are implemented and tested. `init`, `scaffold`, `escalate`, and `check` are on the roadmap — see [PLAN.md](./PLAN.md).
...
| `--dry-run` / `--preview` | `convert` | Show a diff without writing. |
