...
`README.md` and `CHANGELOG.md` are also published as a site (`site/`), built with the docsite CLI. It has its own `package.json`, so `npm install` at the root does not install it, and the CLI is not a dependency of it either: the `site/` scripts run it through `npx` at a pinned version. The CLI lives in GitHub Packages, so running the site needs a token with `read:packages` in `NPM_GITHUB_TOKEN`; nothing else in this repo does.
